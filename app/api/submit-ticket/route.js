...
async function buildInvoicePdf(data, invoiceNum) {
  ...
  items.forEach(function(item, idx) {
    ...
    const useFlatRate = itemRate > 0  // flat rate always trumps hourly
    const rate = useFlatRate ? 0 : hourlyRateVal
    let lineAmt = 0
    if (useFlatRate) lineAmt = itemRate
    else if (rateType === 'hourly' && hrs > 0 && hourlyRateVal > 0) lineAmt = hrs * hourlyRateVal
    grandTotal += lineAmt
    ...
    // Show hours always if entered; show rate col only when using hourly (flat rate trumps)
    text(p1, hrs > 0 ? hrs.toFixed(2) : '-', COL_HRS, y - 10, 9, regular, useFlatRate ? MGRAY : BLACK)
    if (useFlatRate) {
      text(p1, '-', COL_RATE, y - 10, 9, regular, MGRAY)  // flat rate shown in AMOUNT col
...
export async function POST(request) {
  try {
    ...
    const itemRows = (lineItems || []).map((item, idx) => {
      ...
      const rate = useFlatRate ? 0 : (parseFloat(hourlyRate) || 0)
      const amt = useFlatRate ? itemRate : (hrs * rate)
...
export async function POST(request) {
 ...
 + rate.toFixed(2) : '-', COL_RATE, y - 10, 9, regular, BLACK)
    } else {
